...
class StreamingParserEngine:
    ...
    def finish(self) -> list[SemanticEvent]:
        ...
        if self._hold_active:
            # Stream ended before the recovered tool call was validated:
    ...
    def _emit_for_state(self, text: str, token_count: int = 0) -> list[SemanticEvent]:
        ...
        if self._hold_active and self.state == ParserState.TOOL_ARGS:
            self._held_raw.append(text)
            self._held_events.append(SemanticEvent(
                EventType.ARG_VALUE_CHUNK, value=text,
                tool_index=self.tool_index, token_count=token_count,
            ))
            return []
    ...
    def _apply_transition(
        ...
        if self._recovered_tool_call and self.state == ParserState.TOOL_BETWEEN and EventType.TOOL_CALL_START in transition.events:
            transition = replace(transition, validate_tool_name=True, provisional_tool_call=True)
    ...
    def _begin_hold(
        ...
        self._hold_until_end = transition.provisional_tool_call
    ...
    def _resolve_hold(
        ...
        """Keep recovered calls provisional until their invoke closes."""
        name = "".join(self._held_name)
        allowed = self.allowed_tool_names
        valid = allowed is not None and name in allowed
        if self._hold_until_end:
            name_end = self.state == ParserState.TOOL_NAME and transition.next_state == ParserState.TOOL_ARGS
            commit = self.state == ParserState.TOOL_ARGS and transition.commit_provisional_tool_call
            if valid and (name_end or commit):
