...
def sparse_attn_indexer(
    ...
    if has_decode:
        ...
        candidate_logits_used = (
            os.environ.get("VLLM_SM80_CANDIDATE_MQA") == "1"
            and current_platform.is_cuda()
            and current_platform.get_device_capability() == (8, 0)
            and not use_deep_gemm and not use_fp4_cache
            and candidate_blocks is not None and not candidate_write
            and candidate_block_size == 8 and next_n == 1
            and decode_metadata.max_seq_len >= 32768
        )
        if candidate_logits_used:
            from vllm.v1.attention.ops.candidate_mqa_sm80 import candidate_mqa_logits

            logger.info_once("SM80 candidate-only MQA logits enabled for flattened decode")
            logits = candidate_mqa_logits(
                padded_q_quant_cast, kv_cache, shard_weights, seq_lens, block_table,
                candidate_blocks[row_lo:row_hi], decode_metadata.max_seq_len,
                candidate_block_size,
            )
        ...
        num_rows = logits.shape[0]
        if candidate_blocks is not None and not candidate_logits_used:
