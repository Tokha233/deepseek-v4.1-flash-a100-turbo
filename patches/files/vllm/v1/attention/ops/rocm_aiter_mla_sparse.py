...
def _decode_block_h(num_heads: int) -> int:
    ...
    """
    if not current_platform.is_cuda() or torch.cuda.get_device_capability() != (8, 0):
        return 16
    return min(16, max(8, triton.next_power_of_2(num_heads)))


@functools.lru_cache
def _sm80_decode_small_kv_tile(num_queries: int, num_heads: int) -> bool:
    return (current_platform.is_cuda()
            and torch.cuda.get_device_capability() == (8, 0)
            and num_heads == 16 and num_queries in (192, 384))


@functools.lru_cache
def _decode_warps(num_queries: int, num_heads: int) -> int:
    if _sm80_decode_small_kv_tile(num_queries, num_heads):
        return 4
    if _decode_block_h(8) != 8:
        return 4
    if num_heads == 8 and num_queries in (3, 4, 5, 6, 8, 10, 12, 16, 20, 24, 32, 48, 64, 80, 96, 160, 320):
        return 8
    if num_heads == 16 and num_queries in (5, 6, 10, 12, 20, 24, 40, 48, 80, 96, 160, 192, 320, 384):
        return 8
    return 4
...
def _rocm_sparse_attn_decode_ragged_triton(
    ...
    block_k = 16 if _sm80_decode_small_kv_tile(num_queries, num_heads) else 32
    if _ON_GFX950:
    ...
    if _ON_GFX950:
        ...
        _sparse_attn_decode_partial_kernel[(num_queries, num_splits, heads_blocks)](
            ...
            NUM_SPLITS=num_splits,
            NUM_STAGES=1,
            num_warps=_decode_warps(num_queries, num_heads),
        )
